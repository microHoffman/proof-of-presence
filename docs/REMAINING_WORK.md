...
Choose whether the UI ever offers persistent token approval for CommunityToken deposits or DynamicPriceSale quote-token
purchases. The recommended defaults are permit-first with an exact approval fallback for bookings, and an exact
approval before each sale purchase. If a persistent approval is offered, it must be explicit, explain the spender risk,
...
- Synchronize variable/zero-price booking entitlements, deposits, cancellations, Presence/Sweat issuance, citizenship
  issuance, operator burn and lost-wallet recovery without duplicating transactions.
- Index booking, token, citizenship, and sale events with finality, reorg rollback, backfill, and upgrade ABI boundaries.
...
- Implement CitizenNFT discovery and lifecycle views, and DynamicPriceSale quote, bounded approval, deadline, purchase,
  and receipt handling.
- Read separate access, token, citizenship, stays, sale, policy, and curve clients; do not expose transfer or approval
  controls for non-transferable credentials and tokens.
...
- Rehearse the complete deploy/Safe/verify/import/issue/recover/revoke/quote/buy/book/cancel/token/index/upgrade flow on
  Celo Sepolia, including external-service outages.
